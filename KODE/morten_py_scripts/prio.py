...
class PriorityQueue:
    ...
    def check_heap(self):
        ...
        return valid
    def print_heap_md(self):
        """Print heap as a single-line Markdown table (values on top, indices below)."""
        if len(self.heap) <= 1:
            print("Heap er tom.")
            return
        ...
        header = "|" + "|".join(str(v) for v in values) + "|"
        # Build separator row
        sep = "|" + "|".join("---" for _ in values) + "|"
        # Build index row
        idx_row = "|" + "|".join(str(i) for i in indices) + "|"
...
if __name__ == "__main__":
    ...
    # arr = [None, 7,19,11,22,28,13,26,34,26,42,27,21,14,81,18,66,38,69,27]
    arr = [None, 14,17,16,28,22,65,29,31,30,26,23,89,64,35,32,48,47,46,45]
