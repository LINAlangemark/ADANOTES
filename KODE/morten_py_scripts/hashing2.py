class HashTable:
    def __init__(self, size, method="separate_chaining", hop_size=4, display_as_letters=False):
        ...
        self.duplicates_count = {}  # holder styr på hvor mange gange hvert bogstav er indsat
    ...
    def insert(self, item):
        ...
        key, is_numeric = self._convert_key(item)
        # Tæl dubletter kun for bogstaver
        display_key = item if not is_numeric else str(key)
        if not is_numeric:
            if display_key not in self.duplicates_count:
                self.duplicates_count[display_key] = 1
            else:
                self.duplicates_count[display_key] += 1
            display_key += str(self.duplicates_count[display_key])  # f.eks. E1, E2
    ...
    def print_table(self):
        ...
        for i, slot in enumerate(self.table):
            if slot is None:
                ...
                for key, count in self.duplicates_count.items():
                    if self._convert_to_display(slot) == key:
                        val = key + str(count)
                        break
    ...
    """
    def print_table(self):
        """Print table med korrekt dubletnummer for bogstaver (første er uden tal, resten får 1, 2 osv.) eller tal."""
        print("Index | Value")
        print("--------------")
        
        # midlertidigt tæller dubletter i rækkefølge
        current_counts = {}
        ...
        for i, slot in enumerate(self.table):
            if slot is None:
                ...
                display = self._convert_to_display(slot)
                
                # Hvis det er bogstav (1-26) og display_as_letters=True
                if 1 <= slot <= 26 and self.display_as_letters:
                    if display not in current_counts:
                        current_counts[display] = 0
                        val = display  # første forekomst vises uden tal
                    else:
                        current_counts[display] += 1
                        val = f"{display}{current_counts[display]}"
                else:
                    val = display  # tal vises som de er
    ...
    def print_table_md(self):
        """Print the hash table as a Markdown table."""
        print("\n| Index | Value |")
        print("|-------|-------|")
        ...
        for i, slot in enumerate(self.table):
            ...
            print(f"| {i} | {val} |")
    def print_history_md(self):
        """Print insertion history as a Markdown table."""
        print("\n| Step | Key | Hash | Probes (path) | Collision |")
        print("|------|-----|------|---------------|-----------|")
        ...
        for step, h in enumerate(self.history, start=1):
            ...
            probes_str = " -> ".join(str(p) for p in h['probes'])
            collision = "Yes" if h.get('collision') else "No"
            print(f"| {step} | {key_display} | {h['hash']} | {probes_str} | {collision} |")
            total_probes += len(h['probes'])
            if h.get('collision'):
                total_collisions += 1
...
if __name__ == "__main__":
    ...
    ht.populate_initial({0: 'D', 1: 'H', 2: 'V', 5: 'P', 7: 'X', 8: 'E'})
    # Define your inputs here (numbers or letters, upper/lowercase)
    ...
    """ 
    ht = HashTable(size=13, method="quadratic_probing", display_as_letters=True)
    ...
    ht.print_history_md()
    """ 
    ht = HashTable(size=17, method="quadratic_probing",display_as_letters=True)
    ht.insert('E')
    ht.insert('E')
    ht.insert('E')
    ht.insert('E')
