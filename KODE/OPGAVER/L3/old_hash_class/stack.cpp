...
Stack::Stack(int stack_size): _stack_size{stack_size}, _top(-1){ //top(-1) so we can increment to index 0 on first push
    ...
    std::fill(_stack, _stack + _stack_size, -1);   // ← all slots empty
}
...
void Stack::showStack(){
    ...
    for (int i = 0; i < _stack_size; ++i){ //current top
...
int Stack::peek(){
    //std::cout<<"top: "<< _top<<" TOS: "<<_stack[_top]<<std::endl;
    int peekElement = _stack[_top];

    return peekElement;

}

void Stack::insert(int element, int index){
