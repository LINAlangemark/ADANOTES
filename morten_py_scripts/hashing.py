class HashTable:
    ...
    def search(self, key):
        ...
        if self.method == "separate_chaining":
            ...
            # Search only within the neighborhood for hopscotch (H must match insert)
            H = self.hop_size
            probes = []
            for offset in range(H):
                ...
                if self.table[i] is None:
                    # if an empty slot found within neighborhood, key not present
                    return False, probes
                if self.table[i] == key:
                    return True, probes
            # If not found within neighborhood, assume not present (simplified)
            return False, probes
        else:
            raise NotImplementedError("Hashing method not implemented")
    ...
    def print_history(self):
        ...
        print(f"Load factor : {len(self.history) / self.size:.2f}")
